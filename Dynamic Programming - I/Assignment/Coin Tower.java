...
public class Solution {
	...
	public static String findWinner(int n, int x, int y) {
		...
		for (int i = 2; i <= n; i++) {
			// Beerus wins if dp[i-1] or dp[i-x] or dp[i-y] is 0, i.e, Whis wins when the
			// number of coins is (i-1), (i-x) or (i-y)
			// If none of them are 0, then Beerus cannot win => This means Whis wins for i
			// number of coins
			if (dp[i - 1] == 0) {
				dp[i] = 1;
